...
## Windows 95 boot fix inventory — MOVED

The canonical list of everything required to boot Windows 95 on this machine now lives
in **`docs/win95_boot_fix_inventory.md`**. It is a reference checklist, not a method, so
it is kept out of this file to stop it loading every time something unrelated is being
debugged.

**Read it before re-diagnosing any Win95 boot hang from scratch**, and update it there
rather than adding another dated note.
...
⚠ **Techniques 80-94 are largely the retired IOS `.PDR` effort** (`docs/archive/xtide_pdr_retired/`).
The approach was abandoned for a SCSIPORT `.MPD`, but the techniques generalise and several
carry retractions - which are the most valuable lines in the file. Do not delete them; do
skip them when the problem is not a Win9x storage driver.
...
| the window at 100% | about 2.5 x 3.5 inches on the owner's screen - too small to read | writes `[General] scale` every launch, default `3` (200%, owner's request); `-Scale` overrides |
