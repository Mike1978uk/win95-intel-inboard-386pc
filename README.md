...
| [#27](https://github.com/Mike1978uk/win95-intel-inboard-386pc/issues/27) | LS-120 — data read back from the drive after a DOS ECP write is corrupt. Reproduced on two cartridges and on both the create and overwrite paths, but **the offset varies** (`0x4001` then `0x8800`), so it is not a fixed boundary. The bad data is structured (ASCII and CP437 shade bytes), not noise. ⚠ `FC` reads back through the same drive, so write-corruption and read-corruption are not yet separated. Next: a factory-fresh NOS 120 MB disk |
