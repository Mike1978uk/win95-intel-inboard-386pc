...
| [#42](https://github.com/Mike1978uk/win95-intel-inboard-386pc/issues/42) | T130B: is polled/no-IRQ optimal? **IRQ 2 is free** on this machine - the only spare line, and the card has jumpers. An IRQ moves no bytes (the driver is PIO either way) but **no IRQ is what forecloses #31**: catching a SCSI reselection needs one. A jumper, one INF line and a boot; no build |
