...
| ~~[#18](https://github.com/Mike1978uk/win95-intel-inboard-386pc/issues/18)~~ | **CLOSED 2026-09-08.** Floppy DMA reach fixed (`maxPhys 0x1000 -> 0xFF`, shipped as `HSFLOP_XTDMA.PDR`). Corruption does not reproduce on this machine: 121 KB written twice to different sectors, three binary compares clean, both drives normal. Floppies are **no longer read-only**. Originally observed on a contributor's hardware, and every measurement predates the #25 geometry fix |
