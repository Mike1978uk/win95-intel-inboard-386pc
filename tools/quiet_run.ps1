# Same capture as memsize_sweep.ps1 but keys are sent ONLY to clear the BIOS
# "ERROR. (RESUME = F1 KEY)" prompt, then never again - INBRDPC.SYS's memory diagnostic
# appears to be abortable by a keystroke, which makes a keep-tapping harness read out a
# partial result (functional+bad = 128k at every board size, which is a stopped counter,
# not a memory map).
param(
  [int]$MemSize = 3072,
...
(Get-Content "$VmPath\86box.cfg.template" -Raw) -replace '(?m)^mem_size = .*$',"mem_size = $MemSize" |
  Set-Content "$VmPath\86box.cfg" -NoNewline
$out = "$VmPath\quiet_$MemSize"; Remove-Item $out -Recurse -Force -EA SilentlyContinue
New-Item -ItemType Directory $out | Out-Null
$p = Start-Process $ExePath -ArgumentList @("-P",$VmPath) -WorkingDirectory $VmPath -PassThru `
       -RedirectStandardError "$VmPath\stderr_q$MemSize.txt" -RedirectStandardOutput "$VmPath\stdout_q$MemSize.txt"
...
while ($sw.Elapsed.TotalSeconds -lt $To -and -not $p.HasExited) {
  ...
  if ($w) {
    [QW+R]$r=New-Object QW+R; [void][QW]::GetClientRect($w.MainWindowHandle,[ref]$r)
    $cw=$r.Rt-$r.L; $ch=$r.B-$r.T
    if ($cw -gt 0 -and $ch -gt 0) {
      $bmp=New-Object System.Drawing.Bitmap $cw,$ch
      $g=[System.Drawing.Graphics]::FromImage($bmp); $dc=$g.GetHdc()
      [void][QW]::PrintWindow($w.MainWindowHandle,$dc,3); $g.ReleaseHdc($dc); $g.Dispose()
      ...
      if (-not $seen.ContainsKey($h)) { $seen[$h]=$t; $bmp.Save(("{0}\f{1:0000}.png" -f $out,[int]($t*10)),[System.Drawing.Imaging.ImageFormat]::Png) }
      $ms.Dispose(); $bmp.Dispose()
    }
    if ($t -ge 12 -and $t -le $KeyUntil -and ($t-$lastKey) -ge 4) { [QW]::Tap($w.MainWindowHandle); $lastKey=$t }
  }
  Start-Sleep -Milliseconds 400
}
Get-Process 86Box -EA SilentlyContinue | Stop-Process -Force
Write-Output "mem_size=$MemSize distinct=$((Get-ChildItem $out).Count)"
