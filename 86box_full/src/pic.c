...
/* Set by an accelerator-card device (e.g. Inboard 386/PC) whose host motherboard has a genuine
   8088-class discrete XT PIC wired up unchanged, despite the CPU package itself reporting is286.
   Forces the deferred/near-zero-delay timer_on_auto() IMR-update path used for real 8088-class
   systems, instead of the synchronous update_pending() path 86Box otherwise uses for any is286
   CPU - matching the physical PIC hardware actually present, not the (faster, decoupled)
   CPU's own classification. */
    ...
    if (addr & 0x0001) {
        switch (dev->state) {
            ...
            case STATE_ICW4:
                ...
                break;
            case STATE_NONE:
                /* 2026-07-26: capped sbprov2-hang trace (INBOARD_86BOX_PORT_PLAN.md, "Sound
                   Blaster Pro hang") - answers whether the driver ever unmasks IRQ5 (master
                   PIC IMR bit 5) at all during the Windows 3.11 hang. Remove once root-caused. */
                {
                    static int hits = 0;
                ...
                if ((is286 && !force_xt_imr_timing) || dev->zenith)
                    ...
                    /* Deliberately small but non-zero, unlike the stock formula below (which
                       ...
                       branch - i.e. it never actually schedules anything). Only affects our own
                       forced-XT-PIC-timing accelerator scenario, not genuine 8088-class machines
                       (whose existing behavior via the branch below is left completely
                       unchanged, since it's untested/unproven territory to touch here). */
