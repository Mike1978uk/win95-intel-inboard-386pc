...
void pitx_control_write(pitx_device_t *pit, uint8_t value)
{
    ...
    /* Diagnostic (2026-07-26, Inboard/Mach8 PIT-freeze chase): channel 0 was confirmed
       parked in PITX_WAIT_COUNT/mode=0 for 75+ real seconds of boot on the ibmxt_inboard386
       machine, while pitx_tick_channel() was confirmed being called at a proper high rate -
       meaning either the BIOS's real Mode-3 control-word write for channel 0 never actually
       reaches this device, or it does and something downstream (finalize_write/
       load_counting_element) doesn't take. Log every control-word write, unconditionally,
       capped - this is a config-write path, nowhere near hot enough to need rate-limiting. */
    {
        static int ctrl_write_count = 0;
        if (ctrl_write_count < 2000) {
            ctrl_write_count++;
            fprintf(stderr, "[pitxctrl] #%d value=%02X select=%u rw=%u mode=%u bcd=%u\n",
                    ctrl_write_count, value, select, (value >> 4u) & 3u, (value >> 1u) & 7u, value & 1u);
            fflush(stderr);
        }
    }
...
void pitx_tick_channel(pitx_device_t *pit, unsigned channel)
{
    ...
    /* Diagnostic (2026-07-26, Inboard/Mach8 PIT-freeze chase): pit_timer_over() in pit.c
       was confirmed firing at a plausible real-time rate (~1.4M ticks/sec) while a ROM's
       counter-0 readback stayed frozen at elapsed=0 for 70+ real seconds - so the freeze
       must be here, in whether this function's state machine actually reaches the
       PITX_COUNTING/decrement path for channel 0. One heartbeat log per real second, channel
       0 only, showing state/mode/gate/counting_element directly. */
    if (channel == 0) {
        static time_t last_hb = 0;
        time_t        now_hb  = time(NULL);
        if (now_hb != last_hb) {
            last_hb = now_hb;
            fprintf(stderr, "[pitxstate] ch0 state=%d mode=%d gate=%d ce=%u clocks=%llu\n",
                    (int) c->state, (int) c->mode, c->gate ? 1 : 0, c->counting_element,
                    (unsigned long long) c->clocks);
            fflush(stderr);
        }
    }
