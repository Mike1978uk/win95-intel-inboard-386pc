 ...
 *
 *          Ported from UniPCemu's hardware/inboard.c (SuperFury,
 *          2019-2022), itself derived from disassembly of INBRDPC.SYS
 *          and primary-source correspondence (Al Williams).
 *
 * Authors: Ported for the COMrade/5160 Inboard project, 2026.
 */
#include <stdarg.h>
#include <stdint.h>
#include <stdio.h>
...
/* Second 64 KB of the card's reserved block, immediately below the BIOS one: the video/EGA
   ROM shadow window. The Inboard reserves 128 KB total, NOT 64 KB - UniPCemu (the reference
   implementation this device is ported from) makes this explicit in mmuhandler.c:

     translatedaddr += 0x20000; // Apply reserved memory as well (this is placed at the
                                // beginning) of 128KB!

   and maps the two halves at 0x5F0000 (BIOS ROM) and 0x5E0000 ("Second 64K of reserved
   memory (EGA ROM as documented)"). This port only ever implemented the first half, which
   is why stock INBRDPC.SYS's extended-memory diagnostic reports `bad extended memory: 128k`
   - see the note at the mapping in inboard386_init() below.

   NOTE this is deliberately NOT the same thing as caching the video ROM at 0xC0000, which is
   Intel's opt-in EGACACHE feature that this file correctly leaves alone. The card reserves
   the window regardless of whether anything shadows into it. */
...
inboard386_apply_waitstates(inboard386_t *dev)
{
    ...
    /* Deliberately do NOT hand `value` to 86Box's own cpu_waitstates knob (2026-08-22,
       upstream 86Box/86Box PR #7749). cpu_update_waitstates() (cpu.c) only honours
       cpu_waitstates for "cpu_type >= CPU_286 && cpu_type <= CPU_386DX", so doing so applies
       the wait states on 386-class CPUs while being silently ignored on the 486BL/486DLC-class
       ones - and inboard386_apply_mem_timing() below already overrides the actually-consumed
       variables (cpu_cycles_read/write/prefetch) directly, with proper bus-speed-ratio scaling,
       for EVERY CPU type. Setting cpu_waitstates as well therefore double-throttles memory
       access on exactly the 386DX/386SX parts this card was really used with (compounding with
       io_waitstates/reg_op_waitstates/cpu_rom_prefetch_cycles), which made those CPU choices
       unusably slow - POST completes, then the machine crawls and never visibly finishes
       booting. Keep it zeroed so there is exactly one memory-timing mechanism in play
       regardless of the selected CPU. */
    cpu_waitstates = 0;
    (void) value;
...
/* The video/EGA half of the reserved block. Straight RAM in both directions - the driver's
   ...
   read 0xFF and the block got marked BAD. */
static uint8_t
...
/* Port 0xA1 - fully unclaimed on this machine (no pic2_init(), no other
   ...
   vanish. Behaves exactly as if unclaimed (return 0xFF on read, do nothing
   on write). The [a0a1trace] spy logging that used to live here was removed
   2026-08-23 once the port-0xA0 question it existed for was answered: the
   Inboard's port_a0 shadow and the machine's NMI-mask handler both receive
   every write (86Box chains I/O handlers), and port_a0 is never read back by
   the wait-state logic, so there is no conflict to trace. */
static uint8_t
...
inboard386_init(const device_t *info)
{
    ...
    /* Second half of the card's 128 KB reserved block. Without this, stock INBRDPC.SYS's
       extended-memory diagnostic reports `bad extended memory: 128k` and - because the driver
       rejects the whole pool over any bad block - `functional extended memory: 0k`, i.e. the
       user loses ALL extended memory. Reported on our #11 and upstream 86Box#7638. */
