...
pit_timer_over(void *priv)
{
    ...
    /* Diagnostic (2026-07-26, Inboard/Mach8 PIT-freeze investigation): log how often this
       callback - the thing that actually clocks the PIT counters forward - really fires in
       real wall-clock time. If a busy-wait ROM delay reading counter 0 stays frozen for
       70+ real seconds while CS:PC keeps moving (already confirmed in 386_dynarec.c's
       [pittrace]), this call rate directly shows whether the callback is starved (fires,
       just far too slowly) or effectively dead (stops firing altogether). Capped + rate
       limited so a working/fast-firing case (expected on every normal machine) doesn't
       flood the log. */
    {
        static int      pto_calls        = 0;
        static time_t   pto_t0           = 0;
        static time_t   pto_last_report  = 0;
        pto_calls++;
        if (pto_t0 == 0)
            pto_t0 = time(NULL);
        time_t pto_now = time(NULL);
        if ((pto_now != pto_last_report) && (pto_calls < 5000000)) {
            pto_last_report = pto_now;
            fprintf(stderr, "[pittimerover] t+%llds calls_so_far=%d dev=%p pit_const=%llu\n",
                    (long long) (pto_now - pto_t0), pto_calls, (void *) dev,
                    (unsigned long long) dev->pit_const);
            fflush(stderr);
        }
    }
...
pit_set_clock(uint32_t clock)
{
    ...
    /* Diagnostic (2026-07-26, Inboard/Mach8 investigation - see COMrade_Latest's
       XT_5160_rework_claude/INBOARD_86BOX_PORT_PLAN.md): a live-traced PIT-channel-0
       readback busy-wait in the ATI Mach8 option ROM was found completely frozen (elapsed
       ticks stuck at 0 for 70+ real seconds) on the ibmxt_inboard386 machine at
       cpu_speed=75000000. One-shot dump of every constant this function derives from
       `clock`, to see directly what this machine's real numbers are before instrumenting
       the callback firing rate below. */
    {
        static int pitclock_dumped = 0;
        if (!pitclock_dumped) {
            pitclock_dumped = 1;
            fprintf(stderr, "[pitclock] clock_param=%u cpuclock=%.3f PITCONSTD=%.6f PITCONST=%llu TIMER_USEC=%llu cpu_type=%d\n",
                    clock, cpuclock, PITCONSTD, (unsigned long long) PITCONST,
                    (unsigned long long) TIMER_USEC, cpu_s->cpu_type);
            fflush(stderr);
        }
    }
}
