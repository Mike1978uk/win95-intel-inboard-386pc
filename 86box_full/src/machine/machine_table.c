...
const machine_t machines[] = {
    ...
    },
    {
        ...
        .flags     = MACHINE_FLAGS_NONE,
        .ram       = {
            /* Real card ships exactly 3 discrete configurations, not a
               continuous range: 1MB base alone, 1MB+2MB piggyback (3MB),
               or 1MB+4MB piggyback (5MB) - the piggyback board is either
               the 2MB or the 4MB part, never both at once (see UniPCemu
               MMU.maxsize logic in hardware/inboard.c, and this project's
               own board notes: "4MB community piggyback (1MB+4MB=5MB)").
               step=2048 with min=1024/max=5120 lands exactly on
               {1024, 3072, 5120} - the three real options - matching
               86Box's plain min/max/step spinner without needing a
               separate discrete-value list. */
            .min  = 1024,
            .max  = 5120,
            .step = 2048
        },
        ...
        .kbd_device               = &keyboard_pc_xt_device,
        .fdc_device               = NULL,
        .vid_device               = &mach8_vga_isa_device,  /* ATI Mach8 video (8-bit ISA, CGA mode) */
        .snd_device               = &sb_pro_v2_device,      /* Sound Blaster Pro 2 (I/O 0x220, IRQ 5, DMA 1) */
