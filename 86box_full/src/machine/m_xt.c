...
static const device_config_t ibmxt_config[] = {
    // clang-format off
    {
        ...
        .bios           = {
            ...
            /* Added 2026-07-26 for the Inboard 386/PC project: the 1982-dated ROMs above
               were what made the earlier POST-timing (PIC IMR / IRQ0 / DMA refresh)
               investigation tractable, but INBRDPC.SYS v1.1 (dated 02/17/89) turns out to
               hardcode a 3-byte reference signature at a fixed BIOS offset (F000:E05B) as
               part of its ROM-shadow self-verification, and that signature does not match
               the 1982 ROM's actual bytes there - a genuine ROM-revision mismatch, not an
               emulation bug (see INBOARD_86BOX_PORT_PLAN.md 2026-07-26). Real Inboard
               installations from the 1989 driver era plausibly used a later ROM revision.
               Offering the same 1986 ROM images ibmxt86_config already has lets that be
               tested directly on this machine instead of only the stock non-Inboard one. */
            {
...
machine_ibmxt_inboard386_init(const machine_t *model)
{
    ...
    /* 2026-07-26: the 1986 ROM revisions (added this session, see
       INBOARD_86BOX_PORT_PLAN.md) use two EQUAL-sized 32KB chips that are BOTH genuine,
       required system BIOS content - unlike the 1982 ROMs above them in this same list,
       where the second file (5000027, 8KB) really is optional Cassette BASIC, correctly
       gated behind enable_basic like every other ibmxt-family machine. Live-traced proof
       (2026-07-26): the 1986 ROM's own early POST does a full 64KB checksum-to-zero
       self-test over F0000-FFFFF (F000:F8C8, sums 65536 bytes starting [DS:BX]=F0000:0000,
       HLTs at F000:E0AB if nonzero) - it was HLTing every time, because F0000-F7FFF was
       never loaded (bios_load_linear above only ever fills the *tail* of the buffer per its
       own documented "prepare 64k rom, load N-byte bios at the end" semantics - see rom.c's
       bios_load() comment). Load files[1] unconditionally, real BIOS content, not repeated
       BASIC, only for the two 1986 entries. */
    ...
    /* 2026-07-26: PIT channel-0 pre-arm - TRIED AND REVERTED, see INBOARD_86BOX_PORT_PLAN.md.
       A real pit_devs[0].write() pre-arm here did unfreeze the Mach8 option ROM's own
       PIT-readback delay loop, but caused a cascade of *new* regressions: channel 0
       actually reaching terminal count across this machine's 70+-real-second POST creates a
       genuinely backlogged IRQ0 that lands during later, unrelated PIC/interrupt
       self-tests deep in this same BIOS's POST (confirmed live: a real "101" POST failure,
       traced to a shared IVT status byte getting contaminated by backlogged IRQ0/IRQ1
       events that would never accumulate on real, unaccelerated hardware). Rather than
       chase each individual downstream collision, the fix now lives narrowly scoped in
       386_dynarec.c - it forces the ATI ROM's own delay-loop *comparison* to resolve
       directly, entirely within the option ROM's own C000 segment, without ever arming the
       real system PIT or generating any interrupt at all. Zero blast radius outside that
       one routine. */

    if (enable_5161) {
        fprintf(stderr, "[m_xt] adding ibm_5161_device (port 0x210 receiver-card presence emulation)\n");
        fflush(stderr);
        device_add(&ibm_5161_device);
    } else {
        fprintf(stderr, "[m_xt] ibm_5161_device NOT added (enable_5161=0)\n");
        fflush(stderr);
    }
