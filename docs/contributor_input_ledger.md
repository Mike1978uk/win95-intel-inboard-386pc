...
|---|---|---|---|
| **Michal Necasek** (OS/2 Museum) | Use the BIOS's own IRET at `F000:FF53` instead of injecting a stub at `0x3C0` | ✅ **Verified correct** — `CF` at offset `0x7F53` in both 1986 ROMs. Strictly better: no injected code, no assumption that INT F0h is unused | ❌ **owed** — plan step 3 |
| **Michal Necasek** | Asked whether Win9x could never support the Inboard because the card couldn't hold enough RAM | Unanswered. His framing is probably right — our working setup runs 5 MB, not a period-plausible config | ❌ **owed** |
| **@andrew-hoffman** | XT DMA page register is only 4 bits → the SB Pro DMA buffer must live under 1 MB (#5, 2026-08-20) | 🟡 Untested, but the **strongest live lead** on the project. Fits a page fault in VDMAD's buffer handling well | ❌ not yet — report either way after the `DMABufferIn1MB=Yes` test |
