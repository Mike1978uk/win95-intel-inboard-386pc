...
| **[itsmattkc/MSVC420](https://github.com/itsmattkc/MSVC420)** | GitHub, no licence (Microsoft binaries) | Redistributed Microsoft code; clone beside the repo, never into it | Portable Visual C++ 4.2 command-line tools with the Win32 import libraries and C runtime; the compiler generation the Win95 DDK expects for C VxDs and Win32 tools. Came up in the owner's conversation with Kevin, Nick and Matt, 2026-09-26. Cloned to `..\MSVC420`; `CL.EXE` 10.20 runs on Windows 11 |
| **[JHRobotics/patcher9x](https://github.com/JHRobotics/patcher9x)** - VxDLIB ([doc](https://github.com/JHRobotics/patcher9x/blob/main/doc/VXDLIB_UTF8.txt)) | GitHub, MIT (VxDLIB itself freeware, non-commercial) | Upstream | Clive Turvey's VxDLIB unpacks a W4 `VMM32.VXD` to W3, lists, extracts and repacks the bundled VxDs; patcher9x uses it to patch VMM after Setup. The route to a post-monolith install such as the real CF. From @andrew-hoffman on #5, 2026-08-29; not recorded until 2026-09-26. Untested here |
