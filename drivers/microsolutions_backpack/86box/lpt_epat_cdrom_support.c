...
epat_attach_drive(epat_t *dev)
{
    ...
    /*
     * Either kind of drive may sit behind this bridge - the EPAT is a generic
     * parallel-to-ATAPI adapter, and was sold carrying both removable disks
     * and CD-ROMs. A removable disk wins if both are assigned to the port,
     * because that is the pairing this bridge was written against.
     */
    ...
    if (dev->sd != NULL)
        ...
        dev->sd = (scsi_device_t *) cdrom_get_lpt_device(dev->port);

        if (dev->sd != NULL) {
            dev->tf = ((scsi_cdrom_t *) dev->sd->sc)->tf;
            epat_log(dev->log, "CD-ROM assigned to LPT%i\n", dev->port + 1);
            return 1;
        }
    }
...
epat_data_read(epat_t *dev)
{
    ...
    if (dev->tf->pos == 16)
        epat_log(dev->log, "DATA IN  head %02X %02X %02X %02X %02X %02X %02X %02X "
...
epat_data_write(epat_t *dev, const uint8_t val)
{
    ...
    if ((sc->packet_status == PHASE_DATA_OUT) && (dev->tf->pos == 16))
        epat_log(dev->log, "DATA OUT head %02X %02X %02X %02X %02X %02X %02X %02X "
                           "%02X %02X %02X %02X %02X %02X %02X %02X\n",
...
epat_read_status(void *priv)
{
    ...
    if (dev->cpp_id_len > 0) {
        ...
        /* DIAGNOSTIC 2026-09-13 - remove once the chain scan is proven. */
        epat_log(dev->log, "CPP nibble byte%i %s -> %02X\n",
                 dev->cpp_id_pos, dev->cpp_id_hi ? "hi" : "lo", ret);
        return ret;
    }
