...
def build():
    ...
    m = ["mov dx,%04X" % E, "in al,dx", "and al,34", "out dx,al", "call 03D0",
         ...
         "mov ax,001C", "call 03F0",
         "mov ax,021D", "call 03F0",
    ...
    setup = ["cli", "mov si,1000", "mov cx,0200",
    ...
    l = (blk(0x100, m) + blk(0x300, cpp4) + blk(0x310, cppp) + blk(0x320, fr)
         ...
         + ["e 0800 28 00 00 00 00 00 00 00 01 00 00 00", ""]
         + blk(0x900, ["cli", "mov si,0800", "mov cx,000C",
                       "mov dx,%04X" % B, "mov al,67", "out dx,al",   # w0(0x67)
                       "mov dx,%04X" % C, "mov al,01", "out dx,al",   # w2(1)
                       "mov al,05", "out dx,al",                       # w2(5)
                       "xor bh,bh", "jmp 0930"])
         + blk(0x930, ["mov al,[si]", "inc si",
                       "mov dx,%04X" % B, "out dx,al",                 # w0(buf[k])
                       "mov dx,%04X" % C, "mov al,04", "add al,bh",
                       "out dx,al",                                    # w2(4+ph)
                       "xor bh,01", "dec cx", "jnz 0930", "jmp 0950"])
         + blk(0x950, ["mov dx,%04X" % C, "mov al,07", "out dx,al",    # w2(7)
                       "mov al,04", "out dx,al", "sti", "ret"])        # w2(4)
    ...
    l += ["u 500 5CF", "u 900 95F", "g=100", "d 600 607", "d 1000 102F", "d 11F0 11FF", "q"]
