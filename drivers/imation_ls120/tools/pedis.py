...
Usage:
    python pedis.py imports
    python pedis.py io                     # every in/out with its port
    python pedis.py dis <rva_hex> [count]
    python pedis.py all  > ../SD120PPD.asm
    python pedis.py str                    # printable strings with RVAs
"""
import os
import struct
import sys
...
DEFAULT = os.path.join(HERE, "..", "SD120PPD.MPD.orig")
...
def main():
    p = PE(DEFAULT)
    cmd = sys.argv[1] if len(sys.argv) > 1 else "imports"
    if cmd == "imports":
        imports(p)
    elif cmd == "io":
        io(p)
        ...
        dis(p, int(sys.argv[2], 16), int(sys.argv[3]) if len(sys.argv) > 3 else 40)
    else:
        print(__doc__)
